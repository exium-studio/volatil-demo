// src/design-system/components/map/hooks/use-wfs-clip-result-layer.ts
...
const WFS_CLIP_RESULT_LINE_LAYER_ID = "wfs-clip-result-line";
const WFS_CLIP_BOUNDARY_SOURCE_ID = "wfs-clip-boundary-source";
const WFS_CLIP_BOUNDARY_LINE_LAYER_ID = "wfs-clip-boundary-line";
...
/**
 * Syncs clipped WFS features and the clipping polygon boundary
 * to MapLibre GeoJSON sources, creating layers when needed.
 * Rebuilds everything after a basemap style swap.
 */
export function useWfsClipResultLayer(map: maplibregl.Map | null): void {
  const clippedFeatures = useWfsClipStore((s) => s.clippedFeatures);
  const clippingPolygon = useWfsClipStore((s) => s.clippingPolygon);
  ...
  useEffect(() => {
    ...
    const ensureLayers = () => {
      ...
      if (!map.getSource(WFS_CLIP_RESULT_SOURCE_ID)) {
        map.addSource(WFS_CLIP_RESULT_SOURCE_ID, {
          type: "geojson",
      ...
      // Clipped fill layer
      if (!map.getLayer(WFS_CLIP_RESULT_FILL_LAYER_ID)) {
        map.addLayer(
          {
            id: WFS_CLIP_RESULT_FILL_LAYER_ID,
            type: "fill",
            source: WFS_CLIP_RESULT_SOURCE_ID,
            paint: {
              "fill-color": "#f6a623",
              "fill-opacity": 0.5,
            },
      ...
      if (!map.getLayer(WFS_CLIP_RESULT_LINE_LAYER_ID)) {
        map.addLayer(
          {
            id: WFS_CLIP_RESULT_LINE_LAYER_ID,
            type: "line",
            source: WFS_CLIP_RESULT_SOURCE_ID,
            paint: {
              "line-color": "#d97706",
      ...
      if (!map.getSource(WFS_CLIP_BOUNDARY_SOURCE_ID)) {
        map.addSource(WFS_CLIP_BOUNDARY_SOURCE_ID, {
          type: "geojson",
          data: clippingPolygonRef.current
            ? {
                type: "FeatureCollection",
                features: [clippingPolygonRef.current],
              }
            : EMPTY_FC,
        });
      }
      ...
      if (!map.getLayer(WFS_CLIP_BOUNDARY_LINE_LAYER_ID)) {
        map.addLayer(
          {
            id: WFS_CLIP_BOUNDARY_LINE_LAYER_ID,
            type: "line",
            source: WFS_CLIP_BOUNDARY_SOURCE_ID,
            paint: {
              "line-color": "#3b82f6",
              "line-width": 2,
              "line-dasharray": [4, 2],
    ...
    return () => {
      ...
      [
        WFS_CLIP_RESULT_LINE_LAYER_ID,
        WFS_CLIP_RESULT_FILL_LAYER_ID,
        WFS_CLIP_BOUNDARY_LINE_LAYER_ID,
      ].forEach((id) => {
        if (map.getLayer(id)) map.removeLayer(id);
      });
      [WFS_CLIP_RESULT_SOURCE_ID, WFS_CLIP_BOUNDARY_SOURCE_ID].forEach((id) => {
  ...
  useEffect(() => {
    ...
    const source = map.getSource(WFS_CLIP_RESULT_SOURCE_ID) as
  ...
  useEffect(() => {
    ...
    const source = map.getSource(WFS_CLIP_BOUNDARY_SOURCE_ID) as
      | maplibregl.GeoJSONSource
      | undefined;
