...
type CreateOrderCheckoutResponse = {
  orderId: string;
  orderNumber: string;
  status: "draft" | "pending_payment" | "processing" | "active" | "expired" | "rejected";
  billingCode: string;
  totalPrice: number;
  validatedAt: string;
  expiredAt: string;
...
type OrderDetailResponse = {
  ...
  items: Array<{
    ...
    provisionedLayer?: {
      ...
      status: "queued" | "provisioning" | "ready" | "failed" | "expired" | "revoked";
      validUntil: string;
    };
  }>;
};
...
```
- **Response** *(Raw key hanya dikembalikan sekali dalam response ini, database hanya menyimpan hash)*:
```typescript
...
- **Form Data Fields**:
  ...
  - `transactionId` *(string, optional)*: UUID transaksi/order terkait
  - `orderNumber` *(string, optional)*: Nomor order terkait (contoh: `ORD-2026-00192`)
  - `priority` *(string, optional)*: `low` | `medium` | `high` | `urgent`
  - `category` *(string, optional)*: Kategori isu
  - `files` *(binary array, optional)*: Berkas lampiran gambar, dokumen, atau video
...
- **Endpoint**: `GET /api/v1/mitra/transactions` *(atau internal order history query)*
- **Response**:
```typescript
