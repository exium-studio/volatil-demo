...
const systemExports = ["default", "preset", "system"];
const isRecord = (mod) => {
...
function resolveSystemExport(mod) {
  ...
  const exports$1 = candidate;
  return exports$1.default || exports$1.preset || exports$1.system || candidate;
}
...
function formatInvalidSystemError(file, mod) {
  const exportNames = getExportNames(mod);
  const foundExports = exportNames.length ? `Found export${exportNames.length === 1 ? "" : "s"}: ${exportNames.map(formatExportName).join(", ")}.` : "Found no named exports.";
  return [
    `No Chakra system export found in ${file}.`,
    `The chakra typegen command expects ${systemExports.map(formatExportName).join(
      ", "
